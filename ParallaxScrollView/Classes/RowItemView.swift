  ...
  init(items: [UIView], spacing: ParallaxItemView.Spacing) {
    ...
    items.enumerated()
      .forEach { (index, view) in
        ...
        let leftMargin = (view == items.first ? self.spacing.leftMargin : 0.0)
        let rightMargin = (view == items.last ? self.spacing.rightMargin : 0.0)
        ...
        NSLayoutConstraint.activate([
          ...
          view.leftAnchor.constraint(
            equalTo: left,
            constant: leftMargin + spacing
          ),
        ])
        
        maxHeight = max(maxHeight, size.height)
        
        let width = size.width + leftMargin + spacing + rightMargin
